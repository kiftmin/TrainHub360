...
export function SettingsPage() {
  ...
  return <div className="page-in">
    ...
    : <div className="grid gap-5 xl:grid-cols-[1.1fr_.9fr]">
      <section className="rounded-lg border border-border/80 bg-card p-5"><p className="font-mono text-[10px] uppercase tracking-[.15em] text-muted-foreground">Identity</p><h2 className="mt-1 text-base font-bold">Workspace details</h2>
      ...
      <section className="rounded-lg border border-border/80 bg-card p-5"><p className="font-mono text-[10px] uppercase tracking-[.15em] text-muted-foreground">Governance</p><h2 className="mt-1 text-base font-bold">Data source</h2><p className="mt-3 text-sm text-muted-foreground">Backend API at <span className="font-mono">/api</span> proxied to <span className="font-mono">localhost:4000</span>. Source: <span className="font-mono">{workspace.data?.dataSource}</span>.</p><div className="mt-4 flex items-center gap-2 text-sm"><Gauge className="size-4 text-primary" />JWT session via localStorage <span className="font-mono text-xs">th360_token</span></div></section>
    </div>}
  </div>;
}
