...
export function CalendarPage() {
  ...
  return <div className="page-in">
    <PageHeading eyebrow="Schedule & support" title="Make the next step easy." detail="One operational view for live training, online sessions, and focused 1:1 support." action={<Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><Button><Plus className="size-4" />Book 1:1</Button></DialogTrigger><DialogContent><DialogHeader><DialogTitle>Book a 1:1 session</DialogTitle></DialogHeader><div className="space-y-4 pt-2"><label className="block text-xs font-semibold">Trainer<Input className="mt-2" placeholder="Trainer name" value={form.trainer} onChange={(e) => setForm({ ...form, trainer: e.target.value })} /></label><label className="block text-xs font-semibold">Course (optional)<Input className="mt-2" placeholder="Related course" value={form.course} onChange={(e) => setForm({ ...form, course: e.target.value })} /></label><label className="block text-xs font-semibold">Date<Input className="mt-2" type="date" value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })} /></label><label className="block text-xs font-semibold">Time<Input className="mt-2" type="time" value={form.time} onChange={(e) => setForm({ ...form, time: e.target.value })} /></label><Button className="w-full" onClick={book} disabled={create.isPending}>{create.isPending ? 'Booking…' : 'Confirm booking'}</Button></div></DialogContent></Dialog>} />
