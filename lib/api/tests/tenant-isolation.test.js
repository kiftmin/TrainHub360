...
describe("tenant isolation", () => {
  ...
  test("sessions and bookings leak nothing from org B", async (t) => {
    if (!server) return t.skip("database unreachable");
    await prisma.session.create({ data: { id: `t-sess-a-${stamp}`, title: "Sess A", date: "2026-10-01", time: "10:00", orgId: ids.orgA } });
    await prisma.session.create({ data: { id: `t-sess-b-${stamp}`, title: "Sess B", date: "2026-10-01", time: "10:00", orgId: ids.orgB } });
    await prisma.booking.create({ data: { id: `t-book-a-${stamp}`, trainer: "TA", date: "2026-10-01", time: "10:00", orgId: ids.orgA } });
    await prisma.booking.create({ data: { id: `t-book-b-${stamp}`, trainer: "TB", date: "2026-10-01", time: "10:00", orgId: ids.orgB } });
    try {
      const sessions = await (await get("/sessions")).json();
      const bookings = await (await get("/bookings")).json();
      assert.ok(sessions.every((r) => r.orgId === ids.orgA), "all sessions must belong to org A");
      assert.ok(bookings.every((r) => r.orgId === ids.orgA), "all bookings must belong to org A");
      assert.ok(!sessions.some((r) => r.id === `t-sess-b-${stamp}`));
      assert.ok(!bookings.some((r) => r.id === `t-book-b-${stamp}`));
    } finally {
      await prisma.session.deleteMany({ where: { id: { in: [`t-sess-a-${stamp}`, `t-sess-b-${stamp}`] } } }).catch(() => null);
      await prisma.booking.deleteMany({ where: { id: { in: [`t-book-a-${stamp}`, `t-book-b-${stamp}`] } } }).catch(() => null);
    }
  });

  test("RSVP on an org-B session fails cleanly", async (t) => {
    if (!server) return t.skip("database unreachable");
    await prisma.session.create({ data: { id: `t-sess-c-${stamp}`, title: "Sess C", date: "2026-10-01", time: "10:00", orgId: ids.orgB } });
    try {
      const { confirmRsvp } = await import("../dist/services/rsvp.js");
      await assert.rejects(() => confirmRsvp(`t-sess-c-${stamp}`, "anyone", ids.orgA), /organization/);
      const { syncSessionToCalendars } = await import("../dist/services/calendarSync.js");
      await assert.rejects(() => syncSessionToCalendars(`t-sess-c-${stamp}`, undefined, ids.orgA), /organization/);
    } finally {
      await prisma.session.deleteMany({ where: { id: `t-sess-c-${stamp}` } }).catch(() => null);
    }
  });
