...
describe("edit/delete CRUD", () => {
  ...
  test("programme create and patch; direct delete is gone (approval flow only)", async (t) => {
    ...
    await prisma.programme.delete({ where: { id: created.id } }).catch(() => null);
  });

  test("course patch works; programme isolation holds", async (t) => {
    if (!server) return t.skip("database unreachable");
